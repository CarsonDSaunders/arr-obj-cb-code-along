...
* If condition evaluates to true, firstExpression is executed. What do you think will happen if condition evaluates to false? secondExpression
...
function schoolStatus(status) {
  var response = (status == "Good") ? "Wow, great job kiddo!" : "No more video games!"
  return response;
}
...
//The first parameter will be an number representing an age. 
//The second and third parameters will be callbacks. 
...
// Write two functions, one called add and one called multiply, that each take in 
...
// This function should return an operator invoked with the appropriate arguments.
...
function math( mathNum1, mathNum2, operator) {
    return operator === add ? add( mathNum1, mathNum2 ) : multiply( mathNum1, mathNum2 );
}
